...
export function Providers({ children, enabled = true }: { children: React.ReactNode; enabled?: boolean }) {
  ...
  return enabled ? <CopilotKitProvider runtimeUrl="/api/copilotkit">{children}</CopilotKitProvider> : <>{children}</>;
}
