...
export class FollowupService {
  constructor(
    ...
    private resolveRecord: (id: string) => { id: string; title: string } = findIncident,
  ) {}
  private marker(id: string) { return `agents-everywhere:${this.resolveRecord(id).id}`; }
  async list(incidentId: string) {
    const tag = this.marker(incidentId);
  ...
  async propose(session: string, input: unknown): Promise<Proposal> {
    ...
    const incident = this.resolveRecord(draft.incidentId);
    ...
    const proposal = {
      ...
      title: draft.title,
      description: `${draft.details}\n\nSample context: ${incident.id} — ${incident.title}\n${this.marker(incident.id)}\nfollowup:${actionKey}`,
