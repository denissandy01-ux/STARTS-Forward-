...
export function useWorkplace(incidentId: string, storage = "Ambiguous") {
  ...
  const propose = useCallback(
    async (draft: { incidentId: string; title: string; details: string }) => {
      try {
        ...
        setNotice("Review the exact project below. It has not been saved.");
        setError("");
        return proposal;
      } catch (error) {
        setError(
  ...
  const retrieve = useCallback(async (id: string) => {
    ...
    setNotice(`Retrieved ${result.task.id} from ${storage}.`);
    return result.task;
  }, [storage]);
  const approve = async () => {
    ...
    try {
      ...
      setNotice(`Saved and read back from ${storage}: ${task.id}. Open Shared projects to review it.`);
      await refresh();
    } catch (error) {
      setError(
        error instanceof Error
  ...
  const deny = async () => {
    ...
    try {
      ...
      setNotice("Proposal declined. No project was created.");
    } catch (error) {
      setError(
